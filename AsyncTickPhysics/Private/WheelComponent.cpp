...
FWheelForces UWheelComponent::SimulateWheel(float DeltaTime, UPrimitiveComponent* BodyMesh, float DriveForce, float BrakeForce, float SteeringAngleDeg, const FTransform& WheelWorldTransform)
{
	...
	if (bStationaryHold)
	{
		// Unified standstill behavior: disable slip-generated tire drive and hold chassis still (including on slopes).
		FLongScalar = FMath::Clamp(-VLong * StationaryHoldVelocityDamping, -MaxTireForce, MaxTireForce);
		FLatScalar = FMath::Clamp(-VLat * StationaryHoldVelocityDamping, -MaxTireForce, MaxTireForce);

		if (const UWorld* World = GetWorld())
		{
			const FVector Gravity(0.0f, 0.0f, World->GetGravityZ());
			const FVector GravityOnPlane = FVector::VectorPlaneProject(Gravity, TireUp);
			const float MassShare = CachedNormalLoad / FMath::Max(1.0f, FMath::Abs(World->GetGravityZ()));
			const FVector AntiSlopeForce = -GravityOnPlane * MassShare * StationaryHoldAntiSlopeScale;
			FLongScalar += FVector::DotProduct(AntiSlopeForce, WheelForward);
			FLatScalar += FVector::DotProduct(AntiSlopeForce, WheelRight);
		}

		FLongScalar = FMath::Clamp(FLongScalar, -MaxTireForce, MaxTireForce);
		FLatScalar = FMath::Clamp(FLatScalar, -MaxTireForce, MaxTireForce);
	}
	else
	{
	...
	if (bEnableStaticFrictionLock && !bStationaryHold)
	{
