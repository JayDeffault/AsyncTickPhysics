...
void UWheelComponent::EnsureCollisionMesh()
{
	...
	WheelCollisionMesh->SetupAttachment(this);
...
void UWheelComponent::UpdateContact(float DeltaTime, UPrimitiveComponent* BodyMesh)
{
	...
	const FVector Start = GetComponentLocation() + GetUpVector() * SuspensionUpperLimit;
	const FVector End = GetComponentLocation() - GetUpVector() * SuspensionLowerLimit;
	const FQuat Rot = WheelCollisionMesh->GetComponentQuat();
	...
	WheelCollisionMesh->ComponentSweepMulti(Hits, Start, End, Rot, QueryParams);
	...
	CurrentSuspensionLength = FVector::Distance(Start, ContactPoint) - WheelRadius;
	...
	if (bDebugVehicle)
	{
		DrawDebugLine(GetWorld(), Start, End, FColor::Cyan, false, DeltaTime);
		DrawDebugSphere(GetWorld(), ContactPoint, 8.0f, 8, FColor::Green, false, DeltaTime);
	}
}
...
FWheelForces UWheelComponent::SimulateWheel(float DeltaTime, UPrimitiveComponent* BodyMesh, float DriveForce, float BrakeForce, float SteeringAngleDeg)
{
	...
	const float VelocityAlongSuspension = FVector::DotProduct(BodyMesh->GetPhysicsLinearVelocityAtPoint(ContactPoint), WheelUp);
